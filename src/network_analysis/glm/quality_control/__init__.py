...
def est_contrast_vifs(desmat, contrasts):
    """
    IMPORTANT: This is only valid to use on design
    matrices where each regressor represents a condition
    vs baseline or if a parametrically modulated regressor
    is used the modulator must have more than 2 levels.
    If it is a 2 level modulation, split the modulation
    into two regressors instead.

    Calculates VIF for contrasts based on the ratio
    of the contrast variance estimate using
    the true design to the variance estimate where
    between condition correaltions are set to 0

    desmat : pandas DataFrame, design matrix
    contrasts : dictionary of contrasts,
    key=contrast name, using the desmat column names
    to express the contrasts
    ...
    for contrast_name, contrast_string in contrasts.items():
        ...
        # The folllowing is the "best case" scenario because the between condition
        # regressor correlations are set to 0
        best_var_contrast = (
            contrast_cvec
            @ np.linalg.inv(
                np.multiply(
...
def get_all_contrast_vif(desmat, contrasts):
    ...
    return pd.DataFrame({
        'contrast': list(contrasts.values()),
